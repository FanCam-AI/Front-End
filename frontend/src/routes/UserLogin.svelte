<script>
  ...
  async function loginWithGoogle(event) {
    ...
    const url = `https://fancamai.com/auth/google?platform=${platform}`;
  ...
  async function loginWithApple(event) {
    ...
    const url = `https://fancamai.com/auth/apple`;
    await Browser.open({
      url: "https://fancamai.com/auth/apple",
      windowName: "_blank", // 세션 유지용
      presentationStyle: "popover", // 선택 사항
    });
  }
</script>
