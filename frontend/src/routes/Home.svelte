<script>
   ...
   * @param onProgress - 업로드 진행 상황을 알려주는 콜백 함수
   * @returns filePath - 청크 단위의 비디오 업로드 완료 후 서버에서 리턴하는 서버에 저장된 파일 경로
   */
  async function uploadVideoInChunks(videoFile, onProgress) {
    ...
    const mergeRequest = (formData) => {
      return new Promise((resolve, reject) => {
        fastapi(
          "post",
          `/api/result/merge_video`,
          formData,
          (res) => resolve(res),
          (err) => reject(err),
        );
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      if (onProgress) {
        const ratio = (i + 1) / totalChunks; // 0 ~ 1
        onProgress(ratio);
      }
    }
    ...
    const mergeRes = await mergeRequest(mergeForm);

    if (!mergeRes || !mergeRes.video_path) {
      throw new Error("비디오 병합 실패");
    }
    return mergeRes.video_path;
  }
 ...
 */
  ...
  onMount(async () => {
    ...
    const raw = sessionStorage.getItem("isLoading");
    if (raw) {
  ...
  async function submitRequest(type, detection_model_type) {
    ...
    try {
      ...
      if (
        ...
        let videoFileName = await new Promise(async (resolve, reject) => {
          try {
            const videoPath = await uploadVideoInChunks(
              videoFile,
              (progress) => {
                isUploading = true;
